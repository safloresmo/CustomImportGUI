...
class ImpartFrontend(impartGUI):
    ...
    def __init__(self, fallback_mode: bool = False) -> None:
        ...
        self._loading_profile = False  # Flag para evitar eventos durante carga de perfil
    ...
    def _add_profile_controls(self) -> None:
        """Add profile selector controls to the GUI."""
        # Get the main sizer
        ...
        # Find insertion point (after m_dirPicker_librarypath)
        insert_index = None
        ...
        # Label
        profile_label = wx.StaticText(self, wx.ID_ANY, "Configuration Profile:")
        profile_label.SetMinSize(wx.Size(150, -1))
        profile_label.SetFont(wx.Font(9, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
        ...
        self.m_choice_profile.SetToolTip(
            "Select a saved configuration profile.\n"
            "Each profile stores: paths, library name and environment variable."
        )
        profile_sizer.Add(self.m_choice_profile, 0, wx.ALL, 5)

        # Save button
        self.m_btn_save_profile = wx.Button(self, wx.ID_ANY, "Save As...", wx.DefaultPosition, wx.DefaultSize, 0)
        self.m_btn_save_profile.SetToolTip("Save current configuration as a new profile")
        profile_sizer.Add(self.m_btn_save_profile, 0, wx.ALL, 5)

        # Delete button
        self.m_btn_delete_profile = wx.Button(self, wx.ID_ANY, "Delete", wx.DefaultPosition, wx.DefaultSize, 0)
        self.m_btn_delete_profile.SetToolTip("Delete the selected profile")
        profile_sizer.Add(self.m_btn_delete_profile, 0, wx.ALL, 5)

        # Insertar selector de perfiles
        main_sizer.Insert(insert_index, profile_sizer, 0, wx.EXPAND | wx.ALL, 5)
        insert_index += 1

        # ===== LIBRARY NAME Y ENV VARIABLE =====
        lib_config_sizer = wx.BoxSizer(wx.HORIZONTAL)

        # Library Name
        lib_name_label = wx.StaticText(self, wx.ID_ANY, "Library Name:")
        lib_name_label.SetMinSize(wx.Size(150, -1))
        lib_config_sizer.Add(lib_name_label, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        ...
        self.m_textCtrl_library_name.SetToolTip(
            "Library name for all imported components\n"
            "Example: MictlanTeam, CustomLibrary, MyProject"
        )
        lib_config_sizer.Add(self.m_textCtrl_library_name, 1, wx.ALL | wx.EXPAND, 5)

        # Env Variable
        env_var_label = wx.StaticText(self, wx.ID_ANY, "Env Variable:")
        env_var_label.SetMinSize(wx.Size(100, -1))
        lib_config_sizer.Add(env_var_label, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        ...
        self.m_textCtrl_library_var.SetToolTip(
            "KiCad environment variable for library paths\n"
            "Example: ${Mictlan_Team}, ${CUSTOM_LIBRARY}"
        )
        lib_config_sizer.Add(self.m_textCtrl_library_var, 0, wx.ALL, 5)

        # Insertar campos de configuración
    ...
    def _on_profile_selected(self, event) -> None:
        ...
        # Activar flag para evitar que _on_library_config_changed interfiera
        self._loading_profile = True

        if self.backend.config.load_profile(selected_profile):
            # Actualizar todos los campos de la GUI
            self.m_dirPicker_sourcepath.SetPath(self.backend.config.get_SRC_PATH())
            self.m_dirPicker_librarypath.SetPath(self.backend.config.get_DEST_PATH())
            ...
            self.backend.print_to_buffer(f"  • Variable: {self.backend.config.get_library_variable()}")
            self.backend.print_to_buffer(f"  • Destination: {self.backend.config.get_DEST_PATH()}\n")
    ...
    def _on_library_config_changed(self, event) -> None:
        """Handle changes to library name or environment variable."""
        # Ignorar eventos mientras estamos cargando un perfil
    ...
    def _on_delete_profile(self, event) -> None:
        ...
        if not selected_profile or selected_profile == "Default":
            wx.MessageBox("Cannot delete the Default profile", "Error", wx.OK | wx.ICON_WARNING)
            return

        dlg = wx.MessageDialog(
            self,
            f"Delete profile '{selected_profile}'?",
            "Confirm",
        ...
        if dlg.ShowModal() == wx.ID_YES:
            if self.backend.config.delete_profile(selected_profile):
                ...
                self.m_dirPicker_sourcepath.SetPath(self.backend.config.get_SRC_PATH())
                self.m_dirPicker_librarypath.SetPath(self.backend.config.get_DEST_PATH())
                self.backend.print_to_buffer(f"\n✓ Profile '{selected_profile}' deleted\n")
        dlg.Destroy()
        event.Skip()
